...
def test_frontend_proxies_covers_that_need_a_referer():
    js = read(os.path.join(WEB, "app.js"))
    body = js[js.index("function attachCover"):]
    body = body[:body.index("\nfunction renderCards")]
    assert "cover_needs_referer" in body
    assert "proxy_cover" in body


def test_no_referrer_meta_is_still_present():
    """Removing it would bring back the MangaDex placeholder."""
...
def test_natomanga_retries_the_same_url_instead_of_another_host():
    js = read(os.path.join(WEB, "app.js"))
    body = js[js.index("function attachCover"):]
    body = body[:body.index("\nfunction renderCards")]
...
def test_mangadass_avoids_the_decoy_search():
    ...
    src = code(read(os.path.join(ROOT, "mangadl", "sources", "mangadass.py")))
...
def test_manga18club_uses_search_not_q():
    ...
    src = code(read(os.path.join(ROOT, "mangadl", "sources", "manga18club.py")))
    body = src[src.index("def search"):src.index("def browse")]
...
def test_manga18club_decodes_the_base64_page_list():
    ...
    from mangadl.sources.manga18club import Manga18ClubSource
...
def test_manga18club_ignores_the_placeholder_image():
    src = read(os.path.join(ROOT, "mangadl", "sources", "manga18club.py"))
...
def test_hentaiakane_parses_the_ts_reader_payload():
    from mangadl.sources.hentaiakane import HentaiAkaneSource
...
def test_hentaiakane_uses_the_plural_genres_path():
    ...
    src = read(os.path.join(ROOT, "mangadl", "sources", "hentaiakane.py"))
    body = src[src.index("def browse"):src.index("def genres")]
...
def test_adult_sources_are_all_rating_stamped():
    ...
    for cls in SOURCE_CLASSES:
        if not getattr(cls, "adult_only", False):
            continue
        src = read(os.path.join(ROOT, "mangadl", "sources",
