...
def home(tmp_path, monkeypatch):
    ...
    from mangadl import config, servercfg
...
def test_no_randomised_token_at_launch():
    ...
    phone every restart."""
    source = read(os.path.join(ROOT, "server.py"))
...
def test_the_settings_panel_has_the_controls():
    html = read(os.path.join(WEB, "index.html"))
    for node in ("setServerToken", "setServerPort", "setServerVerbose",
                 "genServerToken", "copyServerLink"):
        assert f'id="{node}"' in html, node


def test_the_phone_server_card_is_its_own_card():
    """It was accidentally nested inside the Background card: my edit ate
    a closing </div>, and the panel rendered as a box inside a box."""
    from bs4 import BeautifulSoup

    soup = BeautifulSoup(read(os.path.join(WEB, "index.html")), "html.parser")
    token = soup.find(id="setServerToken")
    card = token.find_parent(class_="settings-card")
    assert card is not None
    assert card.find_parent(class_="settings-card") is None, (
        "the Phone server card is nested inside another settings card")
    other = soup.find(id="setLogAdvanced").find_parent(class_="settings-card")
    assert other is not card, "it merged into the Background card"


def test_the_settings_page_markup_is_balanced():
    """A dropped tag silently reparents everything after it."""
    from bs4 import BeautifulSoup

    html = read(os.path.join(WEB, "index.html"))
    soup = BeautifulSoup(html, "html.parser")
    view = soup.find(id="view-settings")
    assert view is not None
    # Every settings card must be a direct child of the view, not of
    # another card.
    for card in view.select(".settings-card"):
        assert card.find_parent(class_="settings-card") is None, (
            f"nested settings card: {card.get_text(strip=True)[:40]}")


def test_the_settings_controls_are_wired():
    js = read(os.path.join(WEB, "app.js"))
    for handler in ("setServerToken", "genServerToken", "setServerPort",
                    "setServerVerbose", "copyServerLink"):
        assert f'$("{handler}")' in js, handler
...
def test_the_gui_and_the_server_agree(tmp_path, monkeypatch):
    ...
    gui_source = read(os.path.join(ROOT, "mangadl", "gui", "__init__.py"))
...
def test_the_controller_reports_state(tmp_path, monkeypatch):
    ...
    from mangadl import config, servercfg, serverui
    importlib.reload(config)
    importlib.reload(servercfg)
...
def test_docs_mention_landing():
    for name in ("README.md", "FEATURES.md"):
