...
def isolated_home(monkeypatch):
    ...
    import mangadl.tracking as tracking
...
def test_mark_and_unmark_read():
    from mangadl.tracking import mark_read, read_chapters
...
def test_mark_many_and_progress():
    from mangadl.tracking import mark_many, progress_for
...
def test_next_unread():
    from mangadl.tracking import mark_many, next_unread
...
def test_progress_on_empty_chapter_list():
    from mangadl.tracking import progress_for

    assert progress_for("u", [])["percent"] == 0.0


def test_clear_progress_for_one_series():
    from mangadl.tracking import clear_progress, mark_read, read_chapters
...
def test_watch_and_unwatch():
    from mangadl.tracking import get_watchlist, is_watched, unwatch, watch
...
def test_record_check_reports_new_chapters():
    from mangadl.tracking import get_watchlist, record_check, watch
...
def test_record_check_ignores_shrinking_counts():
    from mangadl.tracking import record_check, watch
...
def test_acknowledge_clears_the_new_flag():
    from mangadl.tracking import acknowledge, get_watchlist, record_check, watch
...
def test_watch_preserves_the_original_added_date():
    from mangadl.tracking import get_watchlist, watch
...
def test_check_updates_uses_the_source_layer(monkeypatch):
    ...
    monkeypatch.setattr("mangadl.sources.source_for_url",
...
def test_check_updates_skips_failing_sources(monkeypatch):
    ...
    monkeypatch.setattr("mangadl.sources.source_for_url", boom)
...
def test_notes_and_ratings():
    from mangadl.tracking import get_note, set_note
...
def test_rated_listing_is_sorted():
    from mangadl.tracking import rated, set_note
...
def test_missing_note_returns_a_blank_default():
    from mangadl.tracking import get_note
...
def test_scan_duplicates_finds_identical_files(tmp_path):
    from mangadl.tracking import scan_duplicates
...
def test_disk_usage_sorts_largest_first(tmp_path):
    from mangadl.tracking import disk_usage
...
def test_find_orphans_detects_missing_files(tmp_path):
    ...
    from mangadl.tracking import find_orphans
