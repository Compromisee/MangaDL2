...
def test_run_gui_releases_the_slot_on_every_exit(home):
    """A crashed-out launch must not lock the app out permanently."""
    source = read(os.path.join(ROOT, "mangadl", "gui", "__init__.py"))
    body = source[source.index("def run_gui():"):]
    # Every `return 1` after the claim needs a stop() before it.
    for match in ("pywebview is not installed", "GUI assets not found",
                  "Could not create the application window"):
        chunk = body[:body.index(match)]
        assert "instance.stop()" in chunk[-400:], (
            f"the '{match}' path does not release the single-instance slot")
