...
def test_chapter_range_label(names, expected):
    from mangadl.utils import chapter_range_label
...
def test_chapter_bounds():
    from mangadl.utils import chapter_bounds
...
def _packaged_names(bundle, chapter_count=6, **template_overrides):
    ...
    import mangadl.downloader as downloader
...
def test_chapter_filter_controls_exist():
    html = read(os.path.join(WEB, "index.html"))
    for control in ("chMin", "chMax", "chSearch", "chSort",
                    "chHideDl", "chFilterReset"):
        assert f'id="{control}"' in html


def test_filters_only_affect_display_not_selection_keys():
    """Selections are keyed by real chapter index, so hiding a row must not
    silently drop it from an existing selection."""
    js = read(os.path.join(WEB, "app.js"))
    body = js[js.index("function visibleChapterIndices"):]
    body = body[:body.index("\n}") + 2]
    assert "idx.push(i)" in body        # pushes the true index


def test_bulk_buttons_respect_the_filter():
    js = read(os.path.join(WEB, "app.js"))
    block = js[js.index('$("selectAllBtn")'):]
    block = block[:block.index('$("selectNoneBtn")')]
    assert "visibleChapterIndices()" in block


def test_latest_picks_the_highest_number_not_the_last_row():
    js = read(os.path.join(WEB, "app.js"))
    block = js[js.index('$("selectLatestBtn")'):]
    block = block[:block.index("});") + 3]
    assert "chapterNumber" in block


def test_number_inputs_are_styled():
    css = read(os.path.join(WEB, "style.css"))
    assert '.range-row input[type="number"]' in css
    assert ".mini-check" in css
