...
def member_ids():
    import mangadl.sources.madaranet as madaranet
...
def test_resolve_member_detects_the_capability_not_a_name():
    ...
    """
    from mangadl.sources import SOURCES, resolve_member
...
def test_resolve_member_returns_none_for_plain_ids():
    from mangadl.sources import resolve_member
...
def test_an_api_exception_becomes_a_clean_error(app):
    ...
    api = application.config["MANGADL_API"]
...
def test_the_page_carries_the_bridge(app):
    ...
    with application.test_client() as client:
        ...
        assert html.index("/bridge.js") < html.index("app.js")
...
def test_static_assets_are_served(app):
    application, _ = app
    with application.test_client() as client:
        for name in ("app.js", "style.css", "dropdown.js"):
...
def test_events_reach_the_buffer_from_the_api(app):
    ...
    application, _ = app
    api = application.config["MANGADL_API"]
    buffer = application.config["MANGADL_BUFFER"]
...
def test_mobile_styles_exist():
    css = read(os.path.join(ROOT, "mangadl", "gui", "web", "style.css"))
    assert "@media (max-width: 820px)" in css
    block = css[css.index("@media (max-width: 820px)"):]
    # The rail becomes a bottom bar; a 60px side rail on a 390px phone costs
    # 15% of the width and puts navigation in the hardest corner to reach.
