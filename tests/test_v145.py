...
def test_unpadded_base64_decodes():
    ...
    """
    from mangadl.sources.manhwaread import ManhwaReadSource
...
def test_source_uses_the_tolerant_decoder():
    ...
    src = read(os.path.join(ROOT, "mangadl", "sources", "manhwaread.py"))
...
def test_frontend_bars_are_keyed_per_job():
    js = read(os.path.join(WEB, "app.js"))
    assert "function barKey" in js
    body = js[js.index("function barKey"):js.index("function ensureBar")]
    assert "event.job" in body
    # ensureBar must take the whole event, not a bare chapter name
    assert "function ensureBar(event)" in js


def test_frontend_sums_progress_across_jobs():
    js = read(os.path.join(WEB, "app.js"))
    assert "function totalOf" in js
    assert "function refreshOverall" in js


def test_download_button_is_not_blocked_by_a_running_job():
    """Blocking it would make the cart pointless."""
    js = read(os.path.join(WEB, "app.js"))
    body = js[js.index("function updateDownloadButton"):]
    body = body[:body.index("\n}") + 2]
    assert "state.downloading" not in body
    assert "A download is already running" not in js
...
def test_concurrency_limit_is_clamped(monkeypatch):
    from mangadl import gui as guimod
...
def test_cart_markup_exists():
    html = read(os.path.join(WEB, "index.html"))
    for node in ("cartCard", "cartList", "cartCount", "cartClearBtn",
                 "addCartBtn", "setMaxJobs"):
        assert f'id="{node}"' in html, node


def test_cart_styles_exist():
    css = read(os.path.join(WEB, "style.css"))
    for rule in (".cart-row", ".cart-badge", ".cart-title", ".ac-job",
                 ".dl-buttons", ".cart-badge.stopped"):
...
def test_job_label_only_shows_with_several_jobs():
    """A single download should look exactly as it always has."""
    js = read(os.path.join(WEB, "app.js"))
    body = js[js.index("function ensureBar"):js.index("function removeBar")]
    assert "jobs.size > 1" in body
