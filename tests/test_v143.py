...
def test_empty_body_eventually_raises():
    from mangadl.sources.base import Source, ScrapeError
...
def test_chapter_count_detection(item, expected):
    from mangadl.features import _chapter_count
...
def test_chapter_filter_ui_exists():
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    web = os.path.join(root, "mangadl", "gui", "web")
    html = open(os.path.join(web, "index.html"), encoding="utf-8").read()
    js = open(os.path.join(web, "app.js"), encoding="utf-8").read()
    assert 'id="setMinChapters"' in html
    assert 'id="setMaxChapters"' in html
    assert "min_chapters" in js and "max_chapters" in js
...
def test_webtoons_extracts_title_no():
    ...
    from mangadl.sources.webtoons import WebtoonsSource
...
def test_webtoons_sends_a_referer():
    ...
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    source = open(os.path.join(root, "mangadl", "sources", "webtoons.py"),
                  encoding="utf-8").read()
