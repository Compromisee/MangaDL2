...
def test_aggregate_id_is_not_the_engine_name():
    ...
    from mangadl.sources import SOURCES
    from mangadl.sources.madaranet import MadaraNetSource
...
def test_genre_names_map_back_to_each_installs_own_slug():
    ...
    from mangadl.sources.madaranet import _ManhwaTop, _Toonily
...
def test_aggregate_offline_genres_need_no_network():
    ...
    from mangadl.sources import _offline_genres
...
def test_aggregate_is_not_flagged_cloudflare():
    ...
    from mangadl.sources import SOURCES
    from mangadl.sources.madaranet import _SetsuScans
...
def test_rejected_members_are_documented_not_shipped():
    ...
    doc = read(os.path.join(ROOT, "mangadl", "sources", "madaranet.py"))
...
def test_members_keep_their_measured_quirks():
    ...
    from mangadl.sources.madaranet import (_MangaGG, _MangaOwl, _MangaRead,
                                           _ManhuaTop, _Toonily)
...
def test_fetch_does_not_retry_a_404():
    ...
    from mangadl.sources.base import ScrapeError
    from mangadl.sources.witchscans import WitchScansSource
...
def test_cjk_titles_are_not_destroyed():
    ...
    from mangadl.features import _normalise_title
...
def test_editions_of_the_same_work_still_merge():
    from mangadl.features import _normalise_title as key
