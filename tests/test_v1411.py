...
Both are now handled by ``mangadl.config``: one ``RLock`` and one atomic
...
def test_tui_shares_the_same_store():
    ...
    source = open(os.path.join(os.path.dirname(os.path.dirname(
        os.path.abspath(__file__))), "mangadl", "tui.py"), encoding="utf-8").read()
...
def test_gui_no_longer_writes_settings_by_hand():
    ...
    source = open(os.path.join(root, "mangadl", "gui", "__init__.py"),
                  encoding="utf-8").read()
