...
def test_rate_meter_is_a_rolling_window_not_a_lifetime_average():
    ...
    from mangadl.progress import RateMeter
...
def test_stalled_transfer_reports_zero():
    from mangadl.progress import STALE_AFTER, RateMeter
...
def test_eta_is_none_when_it_cannot_be_known():
    ...
    from mangadl.progress import JobProgress, human_eta
...
def test_formatters():
    from mangadl.progress import human_bytes, human_eta, human_rate
...
def test_engine_reports_bytes_through_the_source():
    ...
    source = read(os.path.join(ROOT, "mangadl", "sources", "base.py"))
...
def test_source_has_a_bytes_hook_defaulting_to_none():
    from mangadl.sources import get_source
...
def test_icon_renders_both_states():
    from mangadl.tray import _build_icon_image
...
def test_start_returns_false_without_a_tray():
    ...
    from mangadl.tray import TrayController, tray_available
...
def test_engine_clears_only_its_own_journal():
    source = read(os.path.join(ROOT, "mangadl", "downloader.py"))
...
def test_journal_writes_are_atomic_and_fsynced():
    ...
    source = read(os.path.join(ROOT, "mangadl", "logs.py"))
...
def test_cli_resume_survives_piped_input():
    ...
    source = read(os.path.join(ROOT, "mangadl", "cli.py"))
