...
* ``mangadl search`` gained ``--type``, ``--status``, ``-n/--limit``,
  ``--sort``, ``--reverse``, ``--json``, ``--urls``, ``--open`` and
  ``--download``.
* ``mangadl menu`` is a progressive, numbered interface that needs nothing
  beyond ``rich`` -- the Textual TUI is an optional extra that is frequently
  not installed.
* ``mangadl tui`` crashed with a raw ``ModuleNotFoundError`` traceback when
...
def test_type_narrowing_keeps_unknown_types():
    ...
    from mangadl.cli import _narrow
...
def test_sort_by_title():
    from mangadl.cli import _sort_results
...
def test_missing_textual_prints_guidance_not_a_traceback():
    result = subprocess.run(
        [sys.executable, "-m", "mangadl.cli", "tui"],
    ...
    assert "mangadl menu" in result.stdout
