"""Progressive interactive menu: ``mangadl menu``.
...
def header():
    ...
    console.print(Panel.fit(
        "[bold]MangaDL[/]  [grey58]interactive menu[/]",
...
def run_menu(argv=None):
    """Entry point for ``mangadl menu``."""
    from .logs import setup_logging
    setup_logging()

    if not sys.stdin.isatty():
        console.print("[yellow]The interactive menu needs a terminal.[/]")
        console.print(f"[{DIM}]Try: mangadl search \"one piece\"[/]")
        return 1
