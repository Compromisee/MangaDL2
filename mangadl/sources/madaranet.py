...
:mod:`mangadl.sources.madara` scrapes it; this module turns the whole set of
...
-----------------------
* :mod:`mangadl.sources.madara` -- the scraping engine. Not a source, never in
  the UI.
* :mod:`mangadl.sources.madarascans` -- *Madara Scans*, an unrelated
  scanlation site that does not even run this theme.

Members
-------
Each entry is a full :class:`~mangadl.sources.madara.MadaraSource` subclass,
...
per-site circuit breaker in :mod:`mangadl.robust` still applies to each member
individually.
