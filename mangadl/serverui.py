...
class ServerController:
    ...
    def start(self):
        ...
        self._thread = threading.Thread(target=run, name="mangadl-flask",
                                        daemon=True)
        self._running = True
        self._thread.start()
...
def run_server_window(host="0.0.0.0", port=None, token=None,
    ...
    window = webview.create_window(
        "MangaDL server", html=PAGE, js_api=controller,
...
<h1><span class="dot" id="dot"></span> MangaDL server</h1>
...
<div class="warnbar" id="noauth" style="display:none">
  Running with no access token — anyone on this network can control MangaDL.
</div>
