"""One MangaDL at a time, and a way to poke the one already running.
...
Since the window is hidden, the obvious way to "reopen" MangaDL -- run it
...
def notify_existing(command="show"):
    """Ask an already-running MangaDL to surface. True if one answered.
...
class InstanceServer:
    ...
    def start(self):
        ...
        self._thread = threading.Thread(target=self._serve,
                                        name="mangadl-instance", daemon=True)
        self._thread.start()
        return True
