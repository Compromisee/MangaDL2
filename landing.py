#!/usr/bin/env python3
"""One window to launch any MangaDL interface.
...
process gets the *system* Python, which has none of MangaDL's dependencies
...
3. ``.venv`` / ``venv`` / ``env`` in the project folder, then in its parent,
   ...
   ``projects/mangadl-checkout/MangaDL`` nesting;
...
logger = logging.getLogger("mangadl.landing")
...
class Launcher:
    ...
    TARGETS = {
        ...
        "menu": ("Terminal menu", ["-m", "mangadl.cli", "menu"], True,
        ...
        "cli": ("Command line", ["-m", "mangadl.cli", "--help"], True,
    ...
    def __init__(self):
        ...
        if self.python_where.startswith("system"):
    ...
    def launch(self, target):
        ...
        # A missing script is a clearer message than whatever Python would
        # say about it three frames deep.
        if script_args and script_args[0].endswith(".py"):
            path = os.path.join(HERE, script_args[0])
            if not os.path.isfile(path):
                self.add("error", f"{script_args[0]} is missing from {HERE}")
                return {"ok": False, "error": f"{script_args[0]} not found"}
            script_args = [path] + script_args[1:]

        if needs_terminal:
            command, shell_hint = terminal_command(self.python, script_args,
                                                   f"MangaDL {label}")
            if command is None:
            ...
            command, shell_hint = [self.python] + script_args, False
...
def run_landing():
    try:
        ...
        print("The launcher window needs pywebview:\n\n"
              ...
              "    python server.py     python -m mangadl.cli menu")
        return 1

    launcher = Launcher()
    webview.create_window("MangaDL", html=PAGE, js_api=launcher,
                          width=780, height=720, min_size=(600, 560),
                          background_color="#0b0a12")
    try:
        webview.start()
    except Exception as exc:
        logger.exception("landing window failed")
